...
//  ---		ESHOP   ----
  ...
  $eshopGoId, $productName, $totalPriceInCents, $variableSymbol, $successURL, $failedURL, $secret, $paymentChannels
  ) {
      ...
      $encryptedSignature = GopayHelper::encrypt(
          GopayHelper::hash(
            GopayHelper::concatPaymentCommand(
              (float) $eshopGoId, $productName, (int) $totalPriceInCents, $variableSymbol, $failedURL, $successURL, $secret)
          ), $secret);
  ...
  $paymentSessionId, $eshopGoId, $variableSymbol, $totalPriceInCents, $productName, $secret
  ) {
    $result = array();
    ...
    try {
      ...
      $sessionEncryptedSignature = GopayHelper::encrypt(
          GopayHelper::hash(
            GopayHelper::concatPaymentSession((float) $eshopGoId, (float) $paymentSessionId, $secret)), $secret);
      ...
      if (
        ...
        && !GopayHelper::checkEshopPaymentStatus(
          $payment_status, 'PAYMENT_DONE', (float) $eshopGoId, $variableSymbol, (int) $totalPriceInCents, $productName, $secret)
        )
        ||
        ($result["code"] == GopayHelper::WAITING
        && !GopayHelper::checkEshopPaymentStatus(
          $payment_status, 'WAITING', (float) $eshopGoId, $variableSymbol, (int) $totalPriceInCents, $productName, $secret)
        )
        ||
        ($result["code"] == GopayHelper::TIMEOUTED
        && !GopayHelper::checkEshopPaymentStatus(
          $payment_status, 'TIMEOUTED', (float) $eshopGoId, $variableSymbol, (int) $totalPriceInCents, $productName, $secret)
        )
        ||
        ($result["code"] == GopayHelper::CANCELED
        && !GopayHelper::checkEshopPaymentStatus(
          $payment_status, 'CANCELED', (float) $eshopGoId, $variableSymbol, (int) $totalPriceInCents, $productName, $secret)
        )
      ) {
        /*
  ...
  public static function createBuyerPayment($buyerGoId, $productName, $totalPriceInCents, $variableSymbol, $successURL, $failedURL, $secret) {
    try {
      ...
      $encryptedSignature = GopayHelper::encrypt(
          GopayHelper::hash(
            GopayHelper::concatPaymentCommand(
              (float) $buyerGoId, $productName, (int) $totalPriceInCents, $variableSymbol, $failedURL, $successURL, $secret)
          ), $secret);
      ...
      $sessionEncryptedSignature = GopayHelper::encrypt(
          GopayHelper::hash(
            GopayHelper::concatPaymentSession(
              (float) $buyerGoId, (float) $paymentSessionId, $secret)
          ), $secret);
